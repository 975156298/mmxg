  ...
  <div class="components-container">
    ...
    <el-table id="out-table" :data="list" v-loading="listLoading" element-loading-text="给我一点时间" border fit
      ...
      <el-table-column align="center" label="凭证">
        <template slot-scope="scope">
          <el-button type="primary" @click="voucherFn(scope.row.voucher)" icon="el-icon-picture"
  ...
  export default {
    ...
    methods: {
      ...
      pagelist(size, page) {
        ...
        axios.get('/admin/refill/list?sort=id,desc&size=' + size + '&page=' + page + '&status=' + status).then((response) => {
      ...
      adopt(id) {
        ...
        this.$confirm('是否给与该用户充值审核通过', '提示', {
          ...
          axios.post('/admin/refill/success', data, {headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'}}).then((response) => {
            console.log(response)
            this.pagelist(this.listQuery.limit, 0)
            this.$message({
              type: 'success',
      ...
      noAdopt(id) {
        ...
        this.$confirm('是否给与该用户充值审核未通过', '提示', {
          ...
          axios.post('/admin/refill/fail', data, {headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=utf-8'}}).then((response) => {
            console.log(response)
            this.pagelist(this.listQuery.limit, 0)
            this.$message({
              type: 'success',
      ...
      exportExcel() {
        ...
        wb.Sheets.Sheet1['!ref'] = "A1:E" + this.listQuery.totalElements + 1;
